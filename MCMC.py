...
The more samples that are taken and recorded (e.g. every generation)
the more the trace plot looks like a "fuzzy caterpillar look" (e.g. good trace plot)
The fewewr samples you take the plot look much more spaced out 

(2) How does the width of the 95% credible interval change as we add/subtract data?

As more data is added the interval will shrink and begins to close in on the true value
as we subract data the interval increases away from the true value

(3) How does the trace plot change if the proposal window becomes much bigger 
or smaller for a given amount of data?
as the proposal window increases the plot will get the the true value faster but will sacrifice a small confidence interval
as the proposal window shrinks the plot never seemed to reach the true value
