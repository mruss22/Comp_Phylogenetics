...
def p_optimize(s,n,pCurr):  #s= number of successes, n= number of trials
#pCurr is has to be replaced by a best value
    """
    ...
    pUp=pCurr+diff
    pDown=pCurr-diff    
    L_pCurr=binomPMF(s,n,pCurr)    
    L_pUp=binomPMF(s,n,pUp)    
    L_pDown=binomPMF(s,n,pDown)
    if (L_pCurr < L_pUp):
        while (L_pCurr < L_pUp):
            pCurr=pUp
            L_pCurr=binomPMF(s,n,pCurr)
